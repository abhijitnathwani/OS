...
void *print_message_function (void *ptr){
  ...
  pthread_exit(pthread_self());  //pthread_exit() immediately exits the current thread
  printf("not to be printed\n");
  //return 0;  //using return causes the stack to be unwound and local variables destroyed, 
}
