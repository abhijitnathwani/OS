...
int main()
{
	...
	for (iIndex = 0; iIndex < 100; iIndex++)
	{
		...
		pthread_create(&serverThread,NULL,sending,NULL);
...
void *sending()
{
	...
	memset(achData,0,sizeof(achData));	// removing file name
	while((fgets(achTemp,sizeof(achTemp),fptr)) != NULL ){
		strcat(achData,achTemp);
	}

    write(iConnectionDis,achData,MAX_CONTAINT);     //write's file containt on connection
